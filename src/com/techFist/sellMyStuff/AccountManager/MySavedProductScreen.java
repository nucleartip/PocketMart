...
public class MySavedProductScreen extends PocketActivity implements ServiceCallback.Receiver {
	...
	private int deleteProductStatus = -404;
    ...
    boolean isDeleteSavedProductRequested = false;
	...
	private ServiceConnection mConnection = new ServiceConnection() {
	    public void onServiceConnected(ComponentName className, IBinder service) {
	    	...
	    	//System.out.println("#### Current Status: " + isDeleteSavedProductRequested);
            if(isBidListRequested){ 
            	isDeleteSavedProductRequested = false; 
            	isBidPostRequested = false;            	
                bidListStatus = localService.onStartCommand(new Intent().putExtras(formData), 1, 4);
                if(bidListStatus == -404)
            	doUnbindService();
	    
            }
            if(isDeleteSavedProductRequested){
            	//System.out.println("##### Executing Saved Product Deletion");
            	isBidListRequested = false;
            	isBidPostRequested = false;            	
            	deleteProductStatus = localService.onStartCommand(new Intent().putExtras(formData), 1, 8);
                if(deleteProductStatus == -404)
            	doUnbindService();            	
            }
            if(isBidPostRequested){
            	...
            	bidPostStatus = localService.onStartCommand(new Intent().putExtras(formData), PocketTrader.MY_SAVED_PRODUCT_DETAIL_SCREEN, 2);
	...
	void doBindService() {
	    bindService(new Intent(MySavedProductScreen.this,ProductsService.class), mConnection, Context.BIND_AUTO_CREATE);
	}
	// Un Binding Service
	void doUnbindService() {
	 ...
	 if(mConnection != null){	
	    ...
	    if(isDeleteSavedProductRequested){
			...
			if(deleteProductStatus == 1){
				customDialog = new CustomDialog(context,"Okay","",getString(R.string.product_deleted));
				final Intent intent = new Intent(context,MySavedProductList.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
	    ...
	    setContentView(R.layout.my_saved_product_detail_screen);
	    getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE,R.layout.common_header);
	     //Register a Exception Handler
	     Thread.setDefaultUncaughtExceptionHandler(new PocketHandler(this,MySavedProductScreen.class));
		...
		activity = this;
		formData = new Bundle();
		productName = (TextView)findViewById(R.id.my_saved_product_detail_title);
		productDate = (TextView)findViewById(R.id.my_saved_product_detail_date);
		productPrice = (TextView)findViewById(R.id.my_saved_product_detail_price);
		productImage = (ImageView)findViewById(R.id.my_saved_product_detail_image);
		productRemover = (Button)findViewById(R.id.my_saved_product_remove);	
		submitBid = (Button)findViewById(R.id.my_saved_product_detail_bid);
		productMap = (Button)findViewById(R.id.my_saved_product_detail_direction);
		contactBtn = (Button)findViewById(R.id.my_saved_product_detail_contact);
		progressGroup = (ViewGroup)findViewById(R.id.my_saved_product_progressGroup);
		noBidFoundMsg = (TextView)findViewById(R.id.my_saved_product_no_bid_found);
		bidListView = (ListView)findViewById(R.id.my_saved_product_bidList);
		bidFetchProgreesBar = (ProgressBar)findViewById(R.id.my_saved_product_progress_bar);
		bidIndicator = (ImageView) findViewById(R.id.my_saved_product_bid_indicator);
	    ...
	    productRemover.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View arg0) {
				...
				dialog.show();
				isDeleteSavedProductRequested = true;
				disconnectMyService();
        ...
        contactBtn.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View arg0) {
				...
				dialog = customDialog.getContactTypeChooserDialog("Choose you Contact Option",PocketTrader.user.getUserName(),
						 product.getOwnerID(),product.getOwnerNumber(),product.getPostedByPhoneOption());
				dialog.show();
			}
	...
	protected void onStart() {
		...
		formData.putString("Key",product.getParentKey());
		formData.putParcelable("Receiver", mReceiver);
		 ////System.out.println("##### Saved Products Screen");
		doBindService();
	}
	...
	public void onReceiveResult(int resultCode, Bundle resultData) {
		...
		if(resultCode == 0){
			bidListStatus = resultData.getInt("Result");
			 ////System.out.println("###### Result Status :" + bidListStatus);
			doUnbindService();
		}
		// Delete Saved product Processing
		if(resultCode == 1){
			deleteProductStatus = resultData.getInt("Result");
			 ////System.out.println("###### Result Status :" + deleteProductStatus);
			doUnbindService();
		}	
		// Posting a Bid
		if(resultCode == 2){
			bidPostStatus =  resultData.getInt("Result");
			 ////System.out.println("#### Bid Post Result :" + bidPostStatus);
			doUnbindService();			
		}		
